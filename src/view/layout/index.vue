<template>
  ...
  >
    ...
    <el-container style="flex: 1 1 100%">
      <el-header style="text-align: right; font-size: 12px">
        <div @click="loginOut" class="toolbar">
          <span>bug制造机</span>
        </div>
        <el-divider />
      </el-header>
      <el-main style="padding: 20px 0px 20px 20px; background: #fff">
        <el-scrollbar>
...
const toPath = function (path: string) {
  router.push(path);
};
...
const state = reactive({
  menu: translateDataToTree(JSON.parse(localStorage.getItem('menus'))),
});
</script>
